...
**`blogs`**: `_id, created_at, updated_at, name, url, rss_url(UNIQUE), is_active, consecutive_failures, last_fetched_at, last_fetch_error`
...
역할별 `*_API_KEY` 변수는 없다 — provider는 `google`/`openrouter` 둘뿐이고 각자 위 공유 키를 쓴다.
