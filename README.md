...
> **2026-08-29 — 백엔드 대격변(Great Rewrite) 컷오버 완료.** Go 게이트웨이 + Python 마이크로서비스 4개 + Kafka 구조를, **FastAPI 모듈러 모놀리스 + Mongo 잡 큐** 하나로 재편했다. 배경과 결정 근거는 [`docs/plan/`](docs/plan/00-overview.md)(ADR 8건 포함), 실행 과정에서 발견·수정한 버그는 [06-migration-steps §Phase 10](docs/plan/06-migration-steps.md)에 기록돼 있다. 알려진 이슈는 [`docs/issues/`](docs/issues/README.md).

### 기술 스택

- **언어**: Python 3.12 (단일 언어 — Go는 완전히 제거됨, [ADR-0002](docs/plan/adr/0002-drop-go.md))
- **웹 프레임워크**: FastAPI + uvicorn (async 전용, [ADR-0003](docs/plan/adr/0003-async-stack.md))
- **데이터베이스**: MongoDB (도메인 데이터 + **잡 큐**, [ADR-0004](docs/plan/adr/0004-mongo-job-queue.md)), Qdrant (벡터 검색)
- **AI**: Google Gemini(요약·임베딩 1순위) / OpenRouter(챗봇·요약 폴백) — 큐레이션과 실시간 헬스체크를 교차한 **LLM 모델 라우터**([ADR-0008](docs/plan/adr/0008-llm-model-router.md)), LangChain/LangGraph 기반
- **패키지 관리**: uv
- **컨테이너**: Docker & Docker Compose (이미지 2개, 프로세스 4개 — 아래 참고)
- **메시지 큐**: 없음. Kafka는 전면 제거하고 Mongo `jobs` 컬렉션으로 대체했다.
...
의존 방향(모듈 경계): `api → {content, users, chat, embedding} → core`. 도메인 간은 `chat → content, users, embedding`만 허용, `content ↔ users`는 금지. 도메인 패키지는 FastAPI를 import하지 않는다. 상세는 [02-directory-structure](docs/plan/02-directory-structure.md).

### 잡 큐 (Kafka 대체, ADR-0004)

Kafka·컨슈머 그룹·토픽·재시도 토폴로지를 전부 걷어내고 Mongo `jobs` 컬렉션 하나로 대체했다.
...
같은 프로세스가 provider가 다른 후보들 사이를 오갈 수 있어야 하므로, `RoutingChatClient`가 요청된 `model_id`를 보고 Gemini 클라이언트와 OpenRouter 클라이언트 중 하나로 정확히 라우팅한다(한 클라이언트가 한 provider만 말할 수 있는 제약 때문에 필요, [06 §10.7](docs/plan/06-migration-steps.md) 참고). `openrouter-free-model-scouter`(별도 개인 프로젝트)가 후보 모델의 실시간 헬스를 제공하고, 실패 시 정적 폴백 목록으로 계속 동작한다. 모델별 성적은 `llm_model_stats`에 기록되어 자동 강등되고 어드민 화면(`/admin/llm-models`)에 노출된다.

## API

새 계약(v2)은 [04-api-v2](docs/plan/04-api-v2.md)에 경로·스키마·에러 코드 체계가 정리돼 있다. 라우터는 `src/techletter/api/v1/`:
...
docker compose -f docker/compose.dev.yml up -d mongo qdrant   # Kafka 없음
...
운영 서버 구성, 컨테이너 4개(api/worker/summary-worker/embedding-worker) + nginx, GitHub Actions 무중단 배포(이미지 빌드 → `up -d --wait` → 스모크 → 실패 시 자동 롤백), 관측 기준선, 런북은 [08-deployment-and-ops](docs/plan/08-deployment-and-ops.md)에 정리돼 있다. 실제 배포 절차(커밋부터 검증까지)는 워크스페이스 루트 `AGENTS.md`("변경사항 배포").

## 더 읽기

- [`docs/README.md`](docs/README.md) — 설계·계획 문서 전체 목차, ADR 8건
- [`docs/issues/README.md`](docs/issues/README.md) — 알려진 이슈
- [`docs/plan/06-migration-steps.md`](docs/plan/06-migration-steps.md) — 대격변 실행 기록(Phase 0~11, 85스텝)
