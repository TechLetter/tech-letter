...
> **🚧 2026-08-28 — 백엔드 대격변(Great Rewrite) 계획 중.**
> Go 게이트웨이 + Python 마이크로서비스 4개를 **FastAPI 모듈러 모놀리스 + 워커 2개**로 재편하는 계획이 [`docs/plan/`](docs/plan/00-overview.md)에 있습니다. 운영·코드 감사에서 발견된 이슈는 [`docs/issues/`](docs/issues/README.md). 아래 내용은 **현행(구) 아키텍처** 설명이며, 마이그레이션 완료 시 전면 재작성됩니다. 구 설계 문서는 [`docs/legacy/`](docs/legacy/README.md)로 이동했습니다.
