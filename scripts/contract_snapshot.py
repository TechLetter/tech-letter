#!/usr/bin/env python3
"""현행 API 응답 '구조'를 캡처해 골든 스냅샷으로 저장한다.

계획 06 Phase 0.2. 값이 아니라 **모양**(키·타입·null 여부·봉투)을 기록해,
새 계약 구현 후 diff를 떠서 "의도한 변경만 일어났는지" 심사하는 데 쓴다.
계약이 전면 재설계되므로(ADR-0007) 완전 일치를 요구하지는 않는다.
