...
// BasicKafkaConsumer Kafka 컨슈머 기본 구현체
type BasicKafkaConsumer struct {
	consumer *kafka.Consumer
	handlers map[events.EventType]EventHandler
}

// NewBasicConsumer 새로운 기본 Kafka 컨슈머 생성
func NewBasicConsumer(kafkaConfig *Config) (Consumer, error) {
	if kafkaConfig == nil {
...
// Subscribe 토픽 구독
func (c *BasicKafkaConsumer) Subscribe(topics []string) error {
...
func (c *BasicKafkaConsumer) RegisterHandler(eventType events.EventType, handler EventHandler) {
	c.handlers[eventType] = handler
	config.Logger.Infof("registered handler for event type: %s", eventType)
}

// Start 컨슈머 시작
func (c *BasicKafkaConsumer) Start(ctx context.Context) error {
...
func (c *BasicKafkaConsumer) processMessage(ctx context.Context, msg *kafka.Message) error {
	// 이벤트 타입 추출
...
// Close 컨슈머 종료
func (c *BasicKafkaConsumer) Close() error {
