...
핵심은 **쿼터 실패를 재시도 횟수로 소모하지 않는 것**이다. 재시도 창이 일일
쿼터 리셋 주기보다 짧으면, 쿼터로 실패한 잡이 재시도를 다 쓰고 dead로
빠져버린다.
"""
...
def dead_retryable_alert(count: int, threshold: int) -> str | None:
    ...
    `permanent`(봇 차단·404)는 상시 발생하는 정상 노이즈지만, `retryable`이
    dead까지 간 건 재시도를 다 써도 안 풀린 문제라 사람이 봐야 한다. 알림
    채널이 따로 없어 구조화 로그로 낸다.
    """
    if count <= threshold:
        return None
