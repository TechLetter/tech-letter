...
현행 게이트웨이(gin)는 `page=abc`를 422로 거절하지 않고 조용히 기본값으로
넘어간다. 프론트가 그 동작에 기대고 있고(04 §1.1), 골든 스냅샷에서도
`page=abc&page_size=xyz` 요청이 200을 반환하는 것이 확인됐다.
FastAPI 기본 검증에 맡기면 422가 되므로 여기서 직접 파싱한다.
"""
