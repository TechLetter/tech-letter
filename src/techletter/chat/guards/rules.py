"""가드 규칙 — Go 게이트웨이와 Python 챗봇에 **따로** 있던 것을 합쳤다.

현행은 같은 목적의 규칙이 두 곳에 흩어져 서로 달랐다(ISSUE-013). 합치면서
두 구현에 공통으로 있던 문제도 고쳤다.

**어순 문제.** 기존 영어 규칙은 `대상 .* 동사` 한 방향만 봤다. 한국어는
동사가 뒤에 오니 자연스럽지만 영어는 `show me the api key`처럼 동사가 앞에
온다. 그래서 영어로 물으면 그냥 통과했다. 여기서는 lookahead로 **어순과
무관하게** "두 요소가 모두 있는가"를 본다.

**과차단 문제.** 기존 한국어 규칙은 `토큰 .* 알려`를 막았다. 기술 블로그
챗봇에서 "JWT 토큰 만료 처리 알려줘"가 정책 위반으로 막혔다는 뜻이다.
값을 내놓으라는 강한 동사(보여/출력/공개/노출)와 약한 동사(알려)를 나누고,
약한 동사는 런타임에 매인 대상(환경변수)에만 적용한다.
"""
