...
현행은 **짧을 때만** 차단 마커를 봤다(soft는 500자 미만, strong은 1000자
미만). 그래서 긴 Cloudflare 챌린지 페이지가 통과해 LLM 쿼터를 태우고
"요약 불가"로 끝났다(ISSUE-007 #2).

여기서는 길이와 무관하게 본다. 대신 soft 마커(“not found” 같은 흔한 단어)만
