...
현행 동작을 그대로 옮긴다: **필터가 걸리면 전체 목록을 기준으로 삼고,
필터 결과에 없는 항목은 `count=0`으로 남긴다.** 프론트가 체크박스 목록을
그대로 유지한 채 개수만 바꾸는 UI라서 그렇다.

"전체 목록"은 blog_id를 무시한다(현행과 동일). 즉 블로그를 고르면 그 블로그에
